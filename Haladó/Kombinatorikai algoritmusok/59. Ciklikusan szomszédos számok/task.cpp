/*
   Ciklikusan szomszédos számok: mint a 58-as feladat, de a legkisebb számot a
   legnagyobb követi, és fordítva.
*/
#include <bits/stdc++.h>
using namespace std;
...
int main() {
    ...
    if (prv.empty()) {
        auto prevLen = largestFixedLen(N, M, M, true);
        cout << join(prevLen) << '\n';
    } else {
        cout << join(prv) << '\n';
    }
    ...
    if (nxt.empty()) {
        auto nextLen = smallestFixedLen(N, M, M, true);
        cout << join(nextLen) << '\n';
    } else {
        cout << join(nxt) << '\n';
    }
